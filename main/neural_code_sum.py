...
def generate_java_doc_template(file_path, method_hypotheses, method_token_list):
	...
	with open(file_path, 'r') as java_file:
		...
		print('===============>',len(method_list),'::',len(method_hypotheses))
		print(method_hypotheses.keys())
		for i, method in enumerate(method_list):
			if method.documentation is not None and len(method.documentation) != 0:
				java_doc.append(None)
				continue
			# print('i:',i,len(method_hypotheses),':',type(method_list))
			if(i < len(method_hypotheses)):
				# print('above')
				print('=>',method.name,'::?::',method_hypotheses[i][0])
...
def append_java_doc(file_path, java_doc_temp, method_token_list,uploadedFile):
	code_with_doc = []
	with open(file_path, 'r') as java_file:
		current_line_no = 0
	...
	with open(str(get_project_root()) + '/' + uploadedFile, 'w') as new_doc_file:
		new_doc_file.truncate(0)
		for code in code_with_doc:
			new_doc_file.write(code)
	# os.system('cmd /c "javadoc PasswordEncryptorUtil.java"')
	htmlDoc.call(uploadedFile)
...
def starter(uploadedFile):
	...
	append_java_doc(args.file_path, java_doc_temp, method_token_list,uploadedFile)
